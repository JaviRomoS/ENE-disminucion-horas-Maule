...
ene22 <- read.csv("https://www.ine.gob.cl/docs/default-source/ocupacion-y-desocupacion/bbdd/bases-anuales/csv/ano-2022.csv?sfvrsn=c55b558c_20&download=true", sep = ";", encoding = "UTF-8")
ene24 <- read.csv("https://www.ine.gob.cl/docs/default-source/ocupacion-y-desocupacion/bbdd/bases-anuales/csv/ano-2024.csv?sfvrsn=97dce357_13&download=true", sep = ";", encoding = "UTF-8")
ene23 <- read.csv("https://www.ine.gob.cl/docs/default-source/ocupacion-y-desocupacion/bbdd/bases-anuales/csv/ano-2023.csv?sfvrsn=40e9c3b5_16&download=true", sep = ";", encoding = "UTF-8")
ene25 <- read.csv("https://www.ine.gob.cl/docs/default-source/ocupacion-y-desocupacion/bbdd/bases-anuales/csv/ano-2025.csv?sfvrsn=ab6fa5e3_4&download=true", sep = ";", encoding = "UTF-8")
...
ene$fact_anual <- as.numeric(gsub(",", ".", ene$fact_anual))

ene <- ene %>% mutate(across(everything(), ~ as.numeric(as.character(.)))) %>% 
  
  mutate(habituales = case_when(habituales == 888 ~ NA_real_,
         ...
         ano22 = case_when(ano_encuesta == 2022 ~ 1,
                           ano_encuesta %in% c(2023, 2024, 2025) ~ 0,
                           TRUE ~ NA_real_),
         ano23 = case_when(ano_encuesta == 2023 ~ 1,
                           ano_encuesta %in% c(2022, 2024, 2025) ~ 0,
                           TRUE ~ NA_real_),
         ano24 = case_when(ano_encuesta == 2024 ~ 1,
                           ano_encuesta %in% c(2022, 2023, 2025) ~ 0,
                           TRUE ~ NA_real_),
         ano25 = case_when(ano_encuesta == 2025 ~ 1,
                           ano_encuesta %in% c(2022, 2023, 2024) ~ 0,
                           TRUE ~ NA_real_),
         
         maule_dummy = case_when(region == "Maule" ~ 1,
                                 region == NA ~ NA,
                                 TRUE ~ 0)) 
